-- ============================================================
-- FabricPro Full Schema for Supabase
...
CREATE TABLE IF NOT EXISTS users (
  ...
  slips_used INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS slips (
  ...
  unit TEXT,
  image_url TEXT,
  linked_slip_id INTEGER,
  rate NUMERIC,
  notes TEXT,
  payment_bill NUMERIC,
...
-- 6. JOB SLIPS
CREATE TABLE IF NOT EXISTS job_slips (
  id SERIAL PRIMARY KEY,
  slip_number TEXT NOT NULL UNIQUE,
  seth_id INTEGER NOT NULL REFERENCES users(id),
  karigar_id INTEGER NOT NULL REFERENCES users(id),
...
-- 7. JOB SLIP ITEMS
CREATE TABLE IF NOT EXISTS job_slip_items (
  id SERIAL PRIMARY KEY,
  slip_id INTEGER NOT NULL REFERENCES job_slips(id),
...
-- 9. RETURN SLIP ENTRIES
CREATE TABLE IF NOT EXISTS return_slip_entries (
  id SERIAL PRIMARY KEY,
  return_slip_id INTEGER NOT NULL REFERENCES return_slips(id),
  job_slip_id INTEGER NOT NULL REFERENCES job_slips(id),
  jama_qty INTEGER NOT NULL DEFAULT 0,
  damage_qty INTEGER NOT NULL DEFAULT 0,
  shortage_qty INTEGER NOT NULL DEFAULT 0,
  no_work_qty INTEGER NOT NULL DEFAULT 0,
  rate_per_pc NUMERIC,
  photo_url TEXT,
  notes TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
-- Disable RLS (since we use our own auth via Bearer token)
